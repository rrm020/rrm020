...
```{r Loading in Data}
RNAivsUntreated_complete <- read_delim("SARTools.DESeq2.genes/tables/RNAivsUntreated.complete.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE)
# Check that numbers agree with SARTools Report
RNAivsUntreated_complete %>% dplyr::filter(padj < 0.05) %>% tally()
RNAivsUntreated_complete %>% dplyr::filter(padj < 0.05) %>% dplyr::filter(FoldChange > 1) %>% tally()
RNAivsUntreated_complete %>% dplyr::filter(padj < 0.05) %>% dplyr::filter(FoldChange < 1) %>% tally()
```
